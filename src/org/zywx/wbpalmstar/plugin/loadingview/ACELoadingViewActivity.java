...
public class ACELoadingViewActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (intent.hasExtra(ELoadingViewUtils.LOADINGVIEW_EXTRA_UEXBASE_OBJ)) {
			mUexBaseObj = (EUExLoadingView) intent
					.getParcelableExtra(ELoadingViewUtils.LOADINGVIEW_EXTRA_UEXBASE_OBJ);
		}
		int styleId = 0;
		if (intent.hasExtra(ELoadingViewUtils.LOADINGVIEW_EXTRA_STYLE_ID)) {
			styleId = intent.getIntExtra(
					ELoadingViewUtils.LOADINGVIEW_EXTRA_STYLE_ID, 0);
		}
		if (intent.hasExtra(ELoadingViewUtils.LOADINGVIEW_EXTRA_POINT_NUM)) {
			mPointNum = intent.getIntExtra(
					ELoadingViewUtils.LOADINGVIEW_EXTRA_POINT_NUM, 0);
		}
		int[] colorArray = new int[] {};
		if (intent.hasExtra(ELoadingViewUtils.LOADINGVIEW_EXTRA_POINT_COLOR)) {
			colorArray = intent
					.getIntArrayExtra(ELoadingViewUtils.LOADINGVIEW_EXTRA_POINT_COLOR);
		}
		...
		if (styleId == 0) {
			...
			for (int i = 0; i < mPointNum; i++) {
				...
				CircleView view = new CircleView(this, radius, color);
				setScale(view, 0.0f);
